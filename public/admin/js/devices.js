                    ...
                    mTable.row.add(
                        [   data[i].code,
                            ...
                            `<div class="d-grid gap-2">
                             <div class="btn btn-lg ${data[i].service_status==0?'btn-danger':'btn-success'}" type="button">
                              ${data[i].service_status==0?'✖':'✓'}
                             </div>
                             </div>`,
                             selectBox
                    ]).draw(false);
...
var ekle = ()=>{
    ...
    if($("#new_model").val()=="-"){
        error_message += "Model seçmelisiniz!<br/>"
    }

    if($("#new_station").val()==""){
        error_message += "Istasyon seçmelisiniz!<br/>"
    }
    ...
    $.ajax({
        ...
        success: (response)=>{
            if(response.status){
                ...
                Swal.fire(
                    'Hata',
                    'Yeni cihaz eklenirken bir hata oluştu!',
                    'error'
                )
            }
            $('#yeniCihazModal').modal('hide');
            formTemizle()
        },
        dataType: 'json',
...
var yeniCihazModal = ()=>{
    ...
    $.ajax({
        ...
        success: (response)=>{
            if(response.status){
                $('#new_code').val(response.data[0].code)
            }else{
                Swal.fire(
                    'Hata',
                    'Form oluşturulurken bir hata oluştu!',
                    'error'
                )
                $('#new_code').val('HATA!!!')
                return
            ...
            if(response.status){
                ...
                for(let i=0;i<response.data.length;i++){
                    $('#new_station').append($('<option>', {
                        ...
                        text: `${response.data[i].name} (${response.data[i].bagli_soket})`
                    }));
                }
            }else{
                Swal.fire(
                ...
                if(response.status){
                    ...
                    $('#detayModal_icerik').html(
                        ...
                        <div class="row mt-2">
                            ...
                            <div class="col-8">
                            <textarea class="form-control" id="description" rows="2">${data.address}</textarea>
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-4">
